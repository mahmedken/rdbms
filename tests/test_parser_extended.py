...
def test_having_without_group_by_fails(clean_catalog):
    ...
    with pytest.raises(ParseException):
        parser.parse("SELECT major, COUNT(*) FROM students HAVING COUNT(*) > 1")
...
def test_parse_negative_limit_fails(clean_catalog):
    ...
    with pytest.raises(ParseException):
        parser.parse("SELECT id, name FROM students LIMIT -5")
...
def test_validation_select_columns_with_group_by(clean_catalog):
    ...
    with pytest.raises(ParseException):
        parser.parse("SELECT name, COUNT(*) FROM students GROUP BY major")
    
    # Invalid: cannot use * with GROUP BY
    with pytest.raises(ParseException):
        parser.parse("SELECT * FROM students GROUP BY major")
